...
import (
  ...
  cmwebhook "github.com/jetstack/cert-manager/pkg/acme/webhook"
...
func NewSolver() cmwebhook.Solver {
  return &aliyunDNSProviderSolver{}
}
...
type aliyunDNSProviderConfig struct {
  ...
  AccessKeyIdRef     cmetav1.SecretKeySelector `json:"accessKeyIdRef"`
  AccessKeySecretRef cmetav1.SecretKeySelector `json:"accessKeySecretRef"`
  TTL                *int                      `json:"ttl"`
}
...
func (c *aliyunDNSProviderSolver) getDnsClient(ch *v1alpha1.ChallengeRequest, cfg aliyunDNSProviderConfig) (*alidns.Client, error) {
  accessKeyId, err := c.getSecretKeyString(ch, &cfg.AccessKeyIdRef)
  if err != nil {
    return nil, err
  }
  accessKeySecret, err := c.getSecretKeyString(ch, &cfg.AccessKeySecretRef)
  if err != nil {
    return nil, err
  }
...
func (c *aliyunDNSProviderSolver) getSecretKeyString(ch *v1alpha1.ChallengeRequest, ref *cmetav1.SecretKeySelector) (string, error) {
  if ref.Key == "" || ref.Name == "" {
    return "",  fmt.Errorf("no SecretKeyRef '%s/%s' in %s", ref.Name, ref.Key, ch.ResourceNamespace)
  }
  ...
  return fmt.Sprintf("%s", secretVal), nil
}
